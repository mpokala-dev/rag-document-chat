  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-shrink-0 flex items-center gap-3 px-6 py-4 border-b border-slate-800 bg-slate-900/50">
        ...
        <div>
          <p className="text-sm font-medium text-slate-200 truncate max-w-md">{documentName}</p>
      ...
      >
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
            <p className="text-slate-400 text-sm">Ask anything about <span className="text-indigo-400">{documentName}</span></p>
            <div className="flex flex-wrap gap-2 justify-center mt-2">
              {['Summarise this document', 'What are the key points?', 'Give me the main requirements'].map(s => (
                <button
                  key={s}
                  onClick={() => { sendMessage({ text: s }); setIsAtBottom(true) }}
